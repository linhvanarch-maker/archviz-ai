...
// ArchViz node accent colors — dùng trong CSS custom properties
// source=#4ade80, render=#a78bfa, material=#fb923c, camera=#38bdf8, panorama=#f472b6
...
function NodeBody({ rfId, data }: { rfId: string; data: FlowboardNodeData }) {
  switch (data.type) {
    ...
    // ── ArchViz AI node types ───────────────────────────────────────────
    case "source":
      return <ImageBody rfId={rfId} data={data} />;   // Source = ảnh tham chiếu, dùng ImageBody
    case "render":
      return <ImageBody rfId={rfId} data={data} />;   // Render = output AI
    case "material":
      return <EditableTextBody rfId={rfId} data={data} variant="prompt" />; // Material = prompt vật liệu
    case "camera":
      return <EditableTextBody rfId={rfId} data={data} variant="note" />; // Camera = góc nhìn/settings
    case "panorama":
      return <ImageBody rfId={rfId} data={data} />;   // Panorama = equirectangular output
  }
}
...
export function NodeCard(props: NodeProps<FlowNode>) {
  ...
  const isGenerable = ["image", "prompt", "video", "visual_asset", "character", "Storyboard", "source", "render", "material", "camera", "panorama"].includes(data.type);
